...
Drives one complete LLM-vs-LLM debate:
  ...
  3. After all turns: calls Judge API (on cluster) for full DL scoring + winner
...
async def _judge_winner(judge_api: str, turns: list[dict], topic: str = "") -> dict:
    """Call Judge API on the cluster for full DL scoring + winner determination."""
    http_turns = [
        {"turn": i + 1, "speaker": t["speaker_id"], "argument": t["text"]}
    ...
    try:
        ...
        log.warning("Judge API failed: %s", exc)
        return {}
